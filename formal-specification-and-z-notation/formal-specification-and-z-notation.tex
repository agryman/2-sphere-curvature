...
\usepackage{formal-specification-and-z-notation}
...
We can work around this limitations by declaring a set of global given sets which are understood to be used in theorem-like
statements where they are taken to represent arbitrary sets.
The article on topological spaces introduces the global given sets $t_1$, $t_2$, and $t_3$ for this purpose.

For example, we can remark that every element $x$ of an arbitrary set $t_1$ is a member of that set.

\begin{remark}

\begin{zed}
	\forall x: t_1 @ x \in t_1
\end{zed}
...
In contrast, the Z notation function application $\intervalRR(a,b)$, defined in the article on real numbers, 
