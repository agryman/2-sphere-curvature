...
\begin{gendef}[t]
	\family: \power t \fun \power(\power(\power t))
\where
	\forall X: \power t @ \family X  = \power(\power X)
\end{gendef}
...
\begin{itemize}
\item The topology is a family of subsets of the set $X$.
...
\begin{gendef}[t]
	top: \power t \fun \power(\family t)
\where
	\forall X: \power t @ \\
		top(X) = \{~ Topology[t] @ \tau ~\}
\end{gendef}
...
\begin{gendef}[t]
	discrete: \power t \fun \family t \\
	indiscrete: \power t \fun \family t
\where
	\forall X: \power t @ discrete(X) = \power X
\also
	\forall X: \power t @ indiscrete(X) = \{ \emptyset, X \}
\end{gendef}
...
The intersection of any set of topologies on $X$ is also a topology on $X$.
...
\begin{gendef}[t]
	topgen: \power t \cross \family t \fun \family t
\where
	\forall X: \power t @ \\
	\t1	\forall B: \family X @ \\
	\t2 		topgen(X,B) = \bigcap \{~ \tau: top(X) | B \subseteq \tau ~\}
\end{gendef}
